...
thread_create(const char *name)
{
	...
	thread->t_name = kstrdup(name);//kstrdup makes a duplicate of a string in dynamically allocated memory
	if (thread->t_name==NULL) {
		kfree(thread);//if the thread has no name, blow it up and kill it irl
		return NULL;
	}
...
mi_switch(threadstate_t nextstate)
{
	...
	assert(result==0);//whatever array it was added to successfully added
